...
local Cat = { Folder = "Cat", Options = {}, ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(117, 164, 206)), ColorSequenceKeypoint.new(0.50, Color3.fromRGB(123, 201, 201)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(224, 138, 175))} }
...
local CatUI = isStudio and script.Parent:WaitForChild("Cat UI") or game:GetObjects("rbxassetid://86467455075715")[1]
...
local function Hide(Window, bind, notif)
	...
	bind = bind[2]
	if notif then
		Cat:Notification({Title = "Interface Hidden", Content = "The interface has been hidden, you may reopen the interface by Pressing the UI Bind In Settings ("..tostring(bind)..")", Icon = "visibility_off"})
	end
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(CatUI)
	CatUI.Parent = CoreGui
elseif not isStudio and CoreGui:FindFirstChild("RobloxGui") then
	CatUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == CatUI.Name and Interface ~= CatUI then
			Hide(Interface.SmartWindow)
			Interface.Enabled = false
			Interface.Name = "Cat-Old"
		end
	end
elseif not isStudio then
...
CatUI.Enabled = false
CatUI.SmartWindow.Visible = false
CatUI.Notifications.Template.Visible = false
CatUI.DisplayOrder = 1000000000

local Main : Frame = CatUI.SmartWindow
local Dragger = Main.Drag
local dragBar = CatUI.Drag
...
local Notifications = CatUI.Notifications
...
-- 	for FlagName, Flag in pairs(Cat.Flags) do
...
-- 		else
-- 			notified = true
-- 			Cat:Notification({Title = "Config Error", Content = "Cat was unable to load or find '"..FlagName.. "'' in the current script. Check ".. website .." for help.", Icon = "flag"})
-- 		end
-- 	end
-- 	if autoload and notified == false then
-- 		Cat:Notification({
-- 			Title = "Config Autoloaded",
-- 			Content = "The Configuration Has Been Automatically Loaded. Thank You For Using Cat Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
...
-- 		Cat:Notification({
-- 			Title = "Config Loaded",
-- 			Content = "The Configuration Has Been Loaded. Thank You For Using Cat Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
-- 	end
...
function Cat:Notification(data) -- action e.g open messages
	task.spawn(function()
		data = Kwargify({
			Title = "Missing Title",
...
function Cat:CreateWindow(WindowSettings)

	WindowSettings = Kwargify({
		Name = "Cat UI Example Window",
		Subtitle = "",
		LogoID = "6031097225",
		LoadingEnabled = true,
		LoadingTitle = "Cat Interface Suite",
	...
	LoadingFrame.Version.Text = LoadingFrame.Frame.Frame.Title.Text == "Cat Interface Suite" and Release or "Cat UI"
	...
	if WindowSettings.KeySystem then
		...
		Draggable(CatUI.MobileSupport, CatUI.MobileSupport)
		...
		local direc = WindowSettings.KeySettings.SaveInRoot and "Cat/Configurations/" .. WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/" or "Cat/Configurations/" ..  WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/"

		if isfile and isfile(direc .. WindowSettings.KeySettings.FileName .. ".Cat") then
			for i, Key in ipairs(WindowSettings.KeySettings.Key) do
				if string.find(readfile(direc .. WindowSettings.KeySettings.FileName .. ".Cat"), Key) then
					Passthrough = true
					break
				end
			end
			...
			KeySystem.Action.Submit.Interact.MouseButton1Click:Connect(function()
				...
				if KeyFound then 
					...
					if WindowSettings.KeySettings.SaveKey then
						if writefile then
							writefile(direc .. WindowSettings.KeySettings.FileName .. ".Cat", FoundKey)
						end
						Cat:Notification({Title = "Key System", Content = "The key for this script has been saved successfully.", Icon = "lock_open"})
					end
				else
	...
	function Window:CreateHomeTab(HomeTabSettings)
		...
		for i,v in pairs(HomeTabSettings.SupportedExecutors) do
			if isStudio then HomeTabPage.detailsholder.dashboard.Client.Subtitle.Text = "Cat Interface Suite - Debugging Mode" break end
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:CreateSection(name : string)
			...
			function Section:CreateButton(ButtonSettings)
				...
				Button.Interact["MouseButton1Click"]:Connect(function()
					...
					if not Success then
						...
						print("Cat Interface Suite | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Button.Title.Text = ButtonSettings.Name
						TweenService:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
			...
			function Section:CreateSlider(SliderSettings, Flag)
						...
						if SLDragging then
							...
							if SliderSettings.CurrentValue ~= NewValue then
								...
								if not Success then
									...
									print("Cat Interface Suite | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
									wait(0.5)
									Slider.Title.Text = SliderSettings.Name
									TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
								...
								-- Cat.Flags[SliderSettings.Flag] = SliderSettings
							end
						else
				...
				if Flag then
					Cat.Options[Flag] = SliderV
				end

				CatUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Slider.Main.color.Color = Cat.ThemeGradient
					Slider.Main.UIStroke.color.Color = Cat.ThemeGradient
				end)
			...
			function Section:CreateToggle(ToggleSettings, Flag)    
				...
				Toggle.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("Cat Interface Suite | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
						TweenService:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
				...
				CatUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Toggle.toggle.color.Color = Cat.ThemeGradient
					Toggle.toggle.UIStroke.color.Color = Cat.ThemeGradient
				end)

				if Flag then
					Cat.Options[Flag] = ToggleV
				end
					...
					if InputSettings.Enter then
						if bleh then
							...
							if not Success then
								...
								print("Cat Interface Suite | "..InputSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Input.Title.Text = InputSettings.Name
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("Cat Interface Suite | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
				...
				function DropdownV:Set(NewDropdownSettings)
					...
					-- Cat.Flags[DropdownSettings.Flag] = DropdownSettings
		...
		function Tab:BuildConfigSection()
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					local success, returned = Cat:SaveConfig(inputPath)
					if not success then
						Cat:Notification({
							Title = "Interface",
							Icon = "error",
					...
					configSelection:Set({ Options = Cat:RefreshConfigList() })
				end
			})
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = Cat:LoadConfig(selectedConfig)
					if not success then
						Cat:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = Cat:SaveConfig(selectedConfig)
					if not success then
						Cat:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					writefile(Cat.Folder .. "/settings/autoload.txt", name)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local name = selectedConfig
					delfile(Cat.Folder .. "/settings/autoload.txt")
			...
			if isfile(Cat.Folder .. "/settings/autoload.txt") then
				local name = readfile(Cat.Folder .. "/settings/autoload.txt")
				loadlabel:Set( { Text = "Current autoload config: " .. name })
			end     
		end
		...
		local ClassParser = {
			["Toggle"] = {
				...
				Load = function(Flag, data)
					if Cat.Options[Flag] then
						Cat.Options[Flag]:Set({ CurrentValue = data.state })
					end
				end
			},
			["Slider"] = {
				...
				Load = function(Flag, data)
					if Cat.Options[Flag] and data.value then
						Cat.Options[Flag]:Set({ CurrentValue = data.value })
					end
				end
			},
			["Input"] = {
				...
				Load = function(Flag, data)
					if Cat.Options[Flag] and data.text and type(data.text) == "string" then
						Cat.Options[Flag]:Set({ CurrentValue = data.text })
					end
				end
			},
			["Dropdown"] = {
				...
				Load = function(Flag, data)
					if Cat.Options[Flag] and data.value then
						Cat.Options[Flag]:Set({ CurrentOption = data.value })
					end
				end
			},
			-- buggy as hell stil
			["Colorpicker"] = {
				...
				Load = function(Flag, data)
					...
					if Cat.Options[Flag] and data.color then
						Cat.Options[Flag]:Set({Color = HexToColor3(data.color)})
					end
				end
			}
		}
		...
		function Tab:BuildThemeSection()
			...
			}, "CatInterfaceSuitePrebuiltCPC1") -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps

			local c2cp = Tab:CreateColorPicker({
				Name = "Color 2",
				Color = Color3.fromRGB(123, 201, 201),
			}, "CatInterfaceSuitePrebuiltCPC2")

			local c3cp = Tab:CreateColorPicker({
				Name = "Color 3",
				Color = Color3.fromRGB(224, 138, 184),
			}, "CatInterfaceSuitePrebuiltCPC3") 
			...
			c1cp:Set({
				Callback = function(Value)
					if c2cp and c3cp then
						Cat.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						CatUI.ThemeRemote.Value = not CatUI.ThemeRemote.Value
					end
				end
			})
			...
			c2cp:Set({
				Callback = function(Value)
					if c1cp and c3cp then
						Cat.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						CatUI.ThemeRemote.Value = not CatUI.ThemeRemote.Value
					end
				end
			})
			...
			c3cp:Set({
				Callback = function(Valuex)
					if c2cp and c1cp then
						Cat.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, Valuex or Color3.fromRGB(255,255,255))}
						CatUI.ThemeRemote.Value = not CatUI.ThemeRemote.Value
					end
				end
			})
		...
		local function SetFolder()
			...
			if WindowSettings.ConfigSettings.RootFolder ~= nil and WindowSettings.ConfigSettings.RootFolder ~= "" then
				Cat.Folder = WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder
			else
				Cat.Folder = WindowSettings.ConfigSettings.ConfigFolder
			end
		...
		function Cat:SaveConfig(Path)
			...
			local fullPath = Cat.Folder .. "/settings/" .. Path .. ".Cat"

			local data = {
				objects = {}
			}

			for flag, option in next, Cat.Options do
		...
		function Cat:LoadConfig(Path)
			...
			local file = Cat.Folder .. "/settings/" .. Path .. ".Cat"
		...
		function Cat:LoadAutoloadConfig()
			if isfile(Cat.Folder .. "/settings/autoload.txt") then
				...
				local success, err = Cat:LoadConfig(name)
				if not success then
					return Cat:Notification({
						Title = "Interface",
						Icon = "sparkle",
						ImageSource = "Material",
		...
		function Cat:RefreshConfigList()
			if isStudio then return "Config system unavailable." end

			local list = listfiles(Cat.Folder .. "/settings")

			local out = {}
			for i = 1, #list do
				local file = list[i]
				if file:sub(-5) == ".Cat" then
					local pos = file:find(".Cat", 1, true)
					local start = pos
	...
	Main.Controls.Close.ImageLabel.MouseButton1Click:Connect(function()
		...
		if UserInputService.KeyboardEnabled == false then
			CatUI.MobileSupport.Visible = true
		end
	end)
	...
	UserInputService.InputBegan:Connect(function(input, gpe)
		...
		if input.KeyCode == Window.Bind then
			...
			CatUI.MobileSupport.Visible = false
	...
	CatUI.MobileSupport.Interact.MouseButton1Click:Connect(function()
		Unhide(Main, Window.CurrentTab)
		dragBar.Visible = true
		Window.State = true
		CatUI.MobileSupport.Visible = false
...
if isStudio then
	local Window = Cat:CreateWindow({
		Name = "Nebula Client - Cat Hub | Blade Ball",
		Subtitle = "by Nebula Softworks",
		LogoID = "123795201100198",
		LoadingEnabled = true,
		LoadingTitle = "Nebula Client (Cat Hub)",
	...
	Cat:Notification({ 
		Title = "Welcome to Cat",
		Icon = "sparkle",
		ImageSource = "Material",
		Content = "Welcome to the Cat Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By Nebula Softworks. Cat was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. Cat Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using Cat Today at " .. website
	})
	...
	Tabs.Main2:CreateDropdown({
		...
		Description = "Cat's Dropdowns Has a built in Player Dropdown!",
...
--[[if isStudio then
    window = Cat:CreateWindow({LoadingEnabled = false})
    t1 = window:CreateTab()
    ...
    t1:CreateParagraph({Text = "Welcome to the Cat Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By Nebula Softworks. Cat was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. Cat Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using Cat Today at " .. website})
